...
function createWorld() {
  ...
  //---
  var el = document.getElementById("codepenelement");
  var elMobile = document.getElementById("codepenelementmobile");
  if (window.innerWidth <= 640) {
    elMobile.appendChild(renderer.domElement);
  } else {
    el.appendChild(renderer.domElement);
  }

  function appendOrRemoveChilds() {
    if (window.innerWidth <= 640) {
      elMobile.appendChild(renderer.domElement);
  ...
  window.addEventListener(["resize"], appendOrRemoveChilds);
